...
func TestAuthenticatedFlow(t *testing.T) {
	...
	ciphertext, nonce, err := box.Encrypt(message, alice.Private, bob.Public)
	if err != nil {
		t.Fatalf("Alice failed to encrypt: %v", err)
	}

	plaintext, err := box.Decrypt(ciphertext, nonce, alice.Public, bob.Private)
	if err != nil {
...
func TestAnonymousFlow(t *testing.T) {
	...
	sealedBox, nonce, err := box.Seal(message, server.Public)
	if err != nil {
		t.Fatalf("Seal failed: %v", err)
	}

	plaintext, err := box.Unseal(sealedBox, nonce, server.Private)
	if err != nil {
...
func TestTampering(t *testing.T) {
	...
	encrypted, nonce, _ := box.Encrypt(msg, alice.Private, bob.Public)

	// Attack 1: Modify Ciphertext
	encrypted[len(encrypted)-1] ^= 0xFF
	_, err := box.Decrypt(encrypted, nonce, alice.Public, bob.Private)
	if err == nil {
	...
	encrypted[len(encrypted)-1] ^= 0xFF
	_, err = box.Decrypt(encrypted, nonce, eve.Public, bob.Private)
	if err == nil {
		t.Error("Decrypt should fail when specifying the wrong sender public key")
	}

	// Sealed Box Tampering
	sealed, nonce, _ := box.Seal(msg, bob.Public)

	sealed[0] ^= 0xFF
	_, err = box.Unseal(sealed, nonce, bob.Private)
	if err == nil {
...
func TestWrongRecipient(t *testing.T) {
	...
	ciphertext, nonce, _ := box.Encrypt(msg, alice.Private, bob.Public)

	_, err := box.Decrypt(ciphertext, nonce,  alice.Public, eve.Private)
	if err == nil {
...
func TestEmptyMessage(t *testing.T) {
	...
	// Authenticated
	ct, nonce, err := box.Encrypt(empty, alice.Private, bob.Public)
	if err != nil {
		t.Fatal(err)
	}
	pt, err := box.Decrypt(ct,nonce,  alice.Public, bob.Private)
	if err != nil {
		t.Fatal(err)
	}
	if len(pt) != 0 {
	...
	// Sealed
	st, nonce, err := box.Seal(empty, bob.Public)
	if err != nil {
		t.Fatal(err)
	}
	pt2, err := box.Unseal(st, nonce, bob.Private)
	if err != nil {
		t.Fatal(err)
	}
	if len(pt2) != 0 {
